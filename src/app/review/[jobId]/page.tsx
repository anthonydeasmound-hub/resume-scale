...
export default function JobReviewPage() {
  ...
  const fetchJob = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setJob({ ...data, reviewed: Boolean(data.reviewed) });
        // Load saved color or default to navy blue
        setAccentColor(data.resume_color && data.resume_color !== "#000000" ? data.resume_color : "#3D5A80");
  ...
  const fetchProfiles = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // If no profile selected yet (and job doesn't have a saved profile), select primary
        if (selectedProfileId === null) {
  ...
  // Handle profile change - reset AI content when profile changes
  const handleProfileChange = async (profileId: number) => {
    if (profileId === selectedProfileId) return;

    // Warn if AI content has been generated
    const hasContent = summaryOptions.length > 0 || selectedRoles.length > 0 || selectedSkills.length > 0;
    if (hasContent) {
      const confirmed = window.confirm(
        "Changing the profile will reset all AI-generated content (summaries, bullets, skills). Do you want to continue?"
      );
      if (!confirmed) return;
    }

    // Reset all AI-generated content
    setSummaryOptions([]);
  ...
  // Load saved tailored resume data when job and master resume are loaded
  useEffect(() => {
