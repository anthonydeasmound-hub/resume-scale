...
export const navyInitialsMetadata: TemplateMetadata = {
  id: 'navy-initials',
  name: 'Navy Initials',
  category: 'professional',
  layout: 'two-column-left',
  description: 'Navy sidebar with initials circle and clean layout',
  supportsPhoto: true,
  supportsSkillBars: false,
  supportsIcons: true,
};

export function generateNavyInitialsHTML(data: ResumeData, options: TemplateOptions): string {
  const { accentColor, showPhoto, showIcons, showLanguages } = options;
  const photoUrl = data.profilePhotoUrl;

  // Get initials from name
  const nameParts = data.contactInfo.name.split(' ');
  const initials = nameParts.length >= 2
    ? `${nameParts[0][0]}${nameParts[nameParts.length - 1][0]}`.toUpperCase()
    : data.contactInfo.name.substring(0, 2).toUpperCase();
  ...
  <div class="page">
    <div class="sidebar">
      <div class="initials-circle">
        ${showPhoto && photoUrl ? `<img src="${photoUrl}" alt="Profile photo" />` : `<span class="initials-text">${initials}</span>`}
      </div>
