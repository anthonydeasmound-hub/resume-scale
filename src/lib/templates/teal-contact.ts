...
export const tealContactMetadata: TemplateMetadata = {
  id: 'teal-contact',
  name: 'Teal Contact',
  category: 'professional',
  layout: 'two-column-left',
  description: 'Teal sidebar with prominent contact icons',
  supportsPhoto: true,
  supportsSkillBars: false,
  supportsIcons: true,
};

export function generateTealContactHTML(data: ResumeData, options: TemplateOptions): string {
  ...
  const contactIcon = (name: string) => {
    ...
    const icons: Record<string, string> = {
      email: '<svg class="contact-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>',
      ...
      location: '<svg class="contact-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>',
...
<head>
  <meta charset="UTF-8">
  <link href="https://fonts.googleapis.com/css2?family=Raleway:wght@400;500;600;700&family=Open+Sans:wght@400;500;600&display=swap" rel="stylesheet">
  <style>
  ...
  <div class="page">
    <div class="sidebar">
      ...
      <div class="sidebar-section">
        ...
        ${data.contactInfo.email ? `<div class="contact-item"><div class="contact-icon-wrapper">${contactIcon('email')}</div><div class="contact-text">${data.contactInfo.email}</div></div>` : ''}
        ${data.contactInfo.phone ? `<div class="contact-item"><div class="contact-icon-wrapper">${contactIcon('phone')}</div><div class="contact-text">${data.contactInfo.phone}</div></div>` : ''}
        ${data.contactInfo.location ? `<div class="contact-item"><div class="contact-icon-wrapper">${contactIcon('location')}</div><div class="contact-text">${data.contactInfo.location}</div></div>` : ''}
        ${data.contactInfo.linkedin ? `<div class="contact-item"><div class="contact-icon-wrapper">${contactIcon('linkedin')}</div><div class="contact-text">${data.contactInfo.linkedin}</div></div>` : ''}
      </div>
