...
export default function RecruiterInfo({
  ...
  const getSourceBadge = () => {
    ...
    return (
      <span className={`text-xs px-2 py-0.5 rounded ${colors[recruiterSource]}`}>
        {labels[recruiterSource]}
      </span>
    );
  };
        ...
        {recruiterName && (
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {recruiterTitle && (
                <div className="text-sm text-gray-500">{recruiterTitle}</div>
              )}
            </div>
          </div>
        )}
