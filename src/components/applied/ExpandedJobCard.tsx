        ...
        <div className="relative bg-white rounded-xl shadow-xl max-w-3xl w-full max-h-[90vh] overflow-hidden">
          <div className="px-6 py-4 border-b bg-gray-50">
            ...
            <div className="mt-2 flex items-center gap-2">
              <span className={`px-2 py-0.5 rounded text-xs font-medium ${job.status === "rejected" ? "bg-red-100 text-red-700" : job.status === "offer" ? "bg-green-100 text-green-700" : job.status === "interview" ? "bg-yellow-100 text-yellow-700" : "bg-blue-100 text-blue-700"}`}>{job.status.charAt(0).toUpperCase() + job.status.slice(1)}</span>
          ...
          <div className="border-b">
            <div className="flex">
              {tabs.map((tab) => (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 transition-colors ${activeTab === tab.id ? "border-blue-600 text-blue-600" : "border-transparent text-gray-600 hover:text-gray-900 hover:border-gray-300"}`}>
                  {tab.icon}{tab.label}
                </button>
              ))}
            </div>
